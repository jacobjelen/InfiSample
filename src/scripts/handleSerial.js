...
function calculate_xyz(data, verbose = false) {
  ...
  // 1600 and 1000 represent a ratio of natural resistances of the sensor in x and y directions
  const x = (xdir * 1600) / act1;
  const y = (ydir * 1000) / act1;
...
function process_for_datalogger(line) {
  ...
  const vals = calculate_xyz(smooth, false);

  _model.touchpad.x = convertRange(vals[0], -300, 300, 0, 1000).toFixed(0);;
  _model.touchpad.y = convertRange(vals[1], -300, 300, 0, 1000).toFixed(0);;
